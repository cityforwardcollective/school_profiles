...
subtitle: |
  | `r glue::glue('{params$house} District {params$district}')`
...
params:
  district: "City"
  representative: "Cavalier Johnson"
  honorific: "Mayor"
  house: "City of Milwaukee"
knitr:
  opts_chunk: 
    R.options:
...
leaflet(dist, options = leafletOptions(zoomSnap = .1, zoomControl = FALSE)) |> 
...
enr_by_rc <- c$c |> 
  ...
  labs(title = "Enrollment by Rating",
       ...
       x = "CFC Rating (2022-23 School Year)") +
...
if (nrow(c_mke$clean_rated) > 0 & (nrow(c_mke$clean_rated) != nrow(c$clean_rated))) {
  c_mke$c <- c_mke$c |> 
  mutate(rating_fill = case_when(
    meets == "No Rating" ~ cfc_colors[4],
    meets == "Not Meeting Expectations" ~ cfc_colors[3],
