...
# prof <- read_rds("../../forward_exam_2020_21/data/wi_all_school_prof.rda")
prof <- read_rds("../../forward_exam_2020_21/data/wi_all_school_test_results.rda")
...
c$c <- c$c |> 
  mutate(rating_fill = case_when(
    meets == "No Rating" ~ cfc_sector_cols[4],
    meets == "Not Meeting Expectations" ~ cfc_sector_cols[3],
    meets == "Meeting Expectations" ~ cfc_sector_cols[2],
    meets == "Exceeding Expectations" ~ cfc_sector_cols[1]
  ))
...
enr_by_sector <- cc |> 
  ...
  geom_col(fill = cfc_dark_green, width = .5) +
...
enr_by_subgroup <- sub_enr |> 
  ...
  geom_col(width = .5, fill = cfc_dark_green) +
...
pp <- prof |> 
  filter(school_year == params$school_year & pa == "pa") |> 
  select(dpi_true_id,
         test_subject,
         perc) |> 
...
pc <- prof |> 
  ...
  select(dpi_true_id,
         test_subject,
         total_count) |> 
  pivot_wider(names_from = test_subject, values_from = total_count, names_prefix = "count_")
